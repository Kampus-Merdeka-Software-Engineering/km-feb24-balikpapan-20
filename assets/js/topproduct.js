...
.then(data => {
    ...
    const productDetails = data.slice(0, 8).map(item => item['product detail']);
    const sales = data.slice(0, 8).map(item => item['Sales']);
    const transactionQtys = data.slice(0, 8).map(item => item['transaction_qty']);

    // Gabungkan kategori dengan detail produk
    const labels = data.slice(0, 8).map(item => `${item['category']} - ${item['product detail']}`);
                ...
                tooltip: {
                    callbacks: {
                        label: function(tooltipItem) {
                            return tooltipItem.dataset.label + ': ' + tooltipItem.raw + ' rb';
                        }
                    }
                },
                datalabels: {
