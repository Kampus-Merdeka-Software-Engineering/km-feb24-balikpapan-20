...
async function generateChart(data) {
  ...
  const totalTransaksiData = data.map((item) =>
    parseInt(item["Total Transaksi"])
  );
  const salesTransaksiData = data.map((item) =>
    parseInt(item["Sales Transaksi"])
  );
...
function updateChartData(data) {
  const totalTransaksiCheckbox = document.getElementById(
    "totalTransaksiCheckbox"
  );
  const salesTransaksiCheckbox = document.getElementById(
    "salesTransaksiCheckbox"
  );

  chart.data.labels = data.map((item) => item.Tanggal);
  chart.data.datasets[0].data = totalTransaksiCheckbox.checked
    ? data.map((item) => parseInt(item["Total Transaksi"]))
    : [];
  chart.data.datasets[1].data = salesTransaksiCheckbox.checked
    ? data.map((item) => parseInt(item["Sales Transaksi"]))
    : [];
  chart.update();
}
